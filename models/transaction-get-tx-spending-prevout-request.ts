/* tslint:disable */
/* eslint-disable */
/**
 * Satstream API
 * Satstream API
 *
 * OpenAPI spec version: 1.0
 * Contact: team@satstream.io
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

import { TxSpendingPrevoutInput } from './tx-spending-prevout-input';
 /**
 * 
 *
 * @export
 * @interface TransactionGetTxSpendingPrevoutRequest
 */
export interface TransactionGetTxSpendingPrevoutRequest {

    /**
     * The transaction outputs to check
     *
     * @type {Array<TxSpendingPrevoutInput>}
     * @memberof TransactionGetTxSpendingPrevoutRequest
     */
    outputs?: Array<TxSpendingPrevoutInput>;
}
